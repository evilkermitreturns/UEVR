...
void VR::on_pre_engine_tick(sdk::UGameEngine* engine, float delta) {
    ...
    auto& ms = ue3d::MonitorState::get();
    {
        ...
        if (!auto_detect_done) {
            auto& bridge = vrmod::VRto3DBridge::get();
    ...
    // Auto-detect physical monitor size (one-shot, only if still at default)
    {
        static bool monitor_size_detected = false;
        if (!monitor_size_detected && ms.bMonitorMode.load(std::memory_order_relaxed)) {
            ...
            if (detected) {
                ...
                if (std::abs(current - 13.24f) < 0.1f) {
                    ...
                    spdlog::info("[UE3D] Auto-detected monitor: {:.1f}cm x {:.1f}cm = {:.1f}\" vertical",
    ...
    if (ms.bMonitorMode.load(std::memory_order_relaxed)) {
        ...
        if (std::isfinite(ipd_cm) && std::isfinite(vert_inches) && std::isfinite(view_dist_cm)
            ...
            const float screen_w_cm = vert_cm * ue3d::constants::DEFAULT_ASPECT;
        ...
        vrmod::VRto3DBridge::get().set_monitor_mode(true);
    }
...
void VR::on_config_load(const utility::Config& cfg, bool set_defaults) {
    ...
    if (!set_defaults) {
        ...
        if (auto v = cfg.get<bool>("gamefov_auto_depth")) gfov_cfg.vrto3d_auto_depth = *v;
        ...
        if (auto v = cfg.get<float>("gamefov_aim_correction")) {
            if (std::isfinite(*v) && *v >= -0.5f && *v <= 0.5f) gfov_cfg.stereo_aim_correction = *v;
        }
        if (auto v = cfg.get<float>("gamefov_aim_base")) {
            if (std::isfinite(*v) && *v >= -0.5f && *v <= 0.5f) gfov_cfg.stereo_aim_base = *v;
        }
        ...
        if (auto v = cfg.get<float>("gamefov_fov_multiplier")) {
            if (std::isfinite(*v) && *v >= 0.1f && *v <= 5.0f) gfov_cfg.fov_multiplier = *v;
        }
...
void VR::on_config_save(utility::Config& cfg) {
    ...
    // Save GameFOV settings
    {
        ...
        cfg.set<bool>("gamefov_auto_depth", gfov_cfg.vrto3d_auto_depth);
        ...
        cfg.set<float>("gamefov_aim_correction", gfov_cfg.stereo_aim_correction);
        cfg.set<float>("gamefov_aim_base", gfov_cfg.stereo_aim_base);
        ...
        cfg.set<float>("gamefov_fov_multiplier", gfov_cfg.fov_multiplier);
        ...
        if (monitor_mode) {
            ...
            if (ImGui::SmallButton("Detect")) {
                ...
                if (detected) {
                    ...
                    spdlog::info("[UE3D] Re-detected monitor: {:.1f}\" vertical", new_vert);
                }
            }
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("Auto-detect from monitor EDID.\n27\" diagonal 16:9 = ~13.2\" vertical.");
            }
            ...
            if (ImGui::Checkbox("Sync Depth to VRto3D", &cfg.vrto3d_auto_depth)) {
                // Smoothing will fade depth back to 1.0 naturally when disabled
            }
            if (ImGui::IsItemHovered()) {
            ...
            if (st.vrto3d_connected) {
                ...
                ImGui::SliderFloat("Base Aim Offset", &cfg.stereo_aim_base, -0.20f, 0.20f, "%.4f");
                if (ImGui::IsItemHovered()) {
                    ImGui::SetTooltip("Constant convergence offset for reticle alignment.\nOnly if bullets land left/right at ALL times.\n\nNote: VR mode feature. Not yet wired for monitor mode.");
                }
                ImGui::SameLine();
                if (ImGui::SmallButton("0##base_aim")) { cfg.stereo_aim_base = 0.0f; }
                if (ImGui::IsItemHovered()) { ImGui::SetTooltip("Reset to 0"); }

                ImGui::SliderFloat("Scoped Aim Correction", &cfg.stereo_aim_correction, -0.20f, 0.20f, "%.4f");
                if (ImGui::IsItemHovered()) {
                    ImGui::SetTooltip("Corrects aim parallax during zoom only.\nAdjust while scoped until bullets hit reticle.\n\nNote: VR mode feature. Not yet wired for monitor mode.");
                }
                ImGui::SameLine();
                if (ImGui::SmallButton("0##scoped_aim")) { cfg.stereo_aim_correction = 0.0f; }
                if (ImGui::IsItemHovered()) { ImGui::SetTooltip("Reset to 0"); }
            }
            ...
            if (ImGui::TreeNode("Status")) {
                ...
                ImGui::Text("  Depth:       %.3f", st.vrto3d_depth);
                ImGui::Text("  Convergence: %.3f", st.vrto3d_convergence);
